...
class TileConfig:
	...
	static func from(data, tile_mode:TileMode) -> TileConfig:
		...
		if data["nav_polygon"] != null:
			# Behold. The worst code I've ever written in public
			print_rich("[color=green]Navigation Polygon[/color]")
			for shrug in data["nav_polygon"]:
				for nav in shrug:
...
class LightOccluder:
	...
	func _init() -> void:
		pass
	static func from(data) -> LightOccluder:
...
class CollisionShape:
	...
	func _init() -> void:
		pass
	static func from(data) -> CollisionShape:
...
func _create_collision_from_shape(tile_data:TileData, shape:CollisionShape, indx:int):
	#print("--- Create Collision From Shape ---")
